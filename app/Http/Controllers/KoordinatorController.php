...
class KoordinatorController extends Controller
{
    ...
    public function updateStatus(Request $request)
    {
        try {
            ...
            if ($request->status_pendaftaran === 'Disetujui') {
                // Jika disetujui, update status tambahan
                // $peserta->status_magang = 'Tidak aktif';
                // $peserta->status_skl = 'Belum diterbitkan';
                ...
                return response()->json(['success' => true, 'message' => 'Pendaftaran berhasil ditolak dan dihapus.']);
            }
    ...
    public function plotMentor(Request $request)
    {
        ...
        try {
            $updated = DB::table('peserta_magangs')
                ->where('nip_peserta', $request->nip_peserta)
                ->update(['nip_mentor' => $request->nip_mentor]);
            $updated = DB::table('penilaians')
                ->where('nip_peserta', $request->nip_peserta)
                ->update(['nip_mentor' => $request->nip_mentor]);

            if ($updated) {
                return response()->json(['message' => 'Mentor berhasil dipilih']);
            } else {
                return response()->json(['error' => 'Gagal memperbarui data'], 500);
            }
        } catch (\Exception $e) {
            return response()->json(['error' => 'Gagal memilih mentor'], 500);
        }
    }
    ...
    public function penilaianPeserta()
    {
        $peserta = DB::table('peserta_magangs')
                    ...
                    ->where('penilaians.status', 'Belum Disetujui')
                    ->get();
    ...
    public function updateNilaiPeserta(Request $request, $nip_peserta)
    {
        ...
        // Hitung nilai_total
        $nilai_total = $request->nilai1 + $request->nilai2 + $request->nilai3 + $request->nilai4 + $request->nilai5 +
                    $request->nilai6 + $request->nilai7 + $request->nilai8 + $request->nilai9 + $request->nilai10;
        ...
        DB::table('penilaians')
            ...
            ->update([
                ...
                'nilai_total' => $nilai_total, // Update nilai_total
            ]);
